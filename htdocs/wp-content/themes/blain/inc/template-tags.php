...
function blain_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'blain' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'blain' ),
...
if ( ! function_exists( 'blain_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function blain_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'blain' ), __( '1 Comment', 'blain' ), __( '% Comments', 'blain' ) );
		echo '</span>';
	}
...
function blain_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'blain_categories' ) ) ) {
		...
		set_transient( 'blain_categories', $all_the_cool_cats );
	}
...
function blain_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? 'blain' : 'parent' ); ?>>
...
function blain_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'blain_categories' );
}
